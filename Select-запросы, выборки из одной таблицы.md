...
tables - er diagram - переходим в скрипт: 
`INSERT INTO <таблица> (<столбики для заполнения>)`
`VALUES('','')`

Если не перечислять атрибуты, тогда придется указать их все в VALUES

`UPDATE <таблица>`
`SET <столбец обновления> = <что обновить>`
`WHERE id = 999;` - условие поиска

`DELETE FROM <таблица>`
`WHERE id = 999;` - условие поиска
Если на что то ссылается, то необходимо сначала удалить ту запись
...
При поиске `%` означает что включает символы без него - четкое совпадение

`DESC` - сортирование по убыванию
